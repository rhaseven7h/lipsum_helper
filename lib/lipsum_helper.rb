...
module LipsumHelper
	def lipsum(params={})
		...
		words_per_sentence = params[:words_per_sentence] || 12
		
		# Set up the Lorem Ipsum text
		words = <<-EOT
			...
			EOT
			# Array-ize the text, and get the count of words
			words = words.downcase.gsub(/[.,]/,' ').split.map{|s| s.strip}
			pool_size = words.size
			
			# Go paragraph by paragraph, sentence by sentence and word by word, stitching them together
			output = '<p>'+1.upto(paragraphs).map do |i| 
				1.upto(sentences_per_paragraph).map do |j|
					1.upto(words_per_sentence).map do |k|
						words[rand(pool_size)]
					end.join(' ').capitalize.strip+'.'
				end.join(' ')
			end.join('</p><p>')+'</p>'
	end
end
